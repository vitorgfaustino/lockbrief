...
- preserve `wrangler.toml` como template publico
...
5. executar apenas etapas automatizaveis e reversiveis
6. parar em checkpoints manuais
7. validar com `npm run dev-init`, `npm run build`, `npm run typecheck` e `npm test` quando houver atualizacao de codigo
8. revisar documentacao correspondente quando comportamento, deploy, operacao ou seguranca mudar
...
| `atualizar_projeto` | `Atualizar o Projeto`, `pull latest version` | fluxo de `docs/ATUALIZACAO.md`, dependencias e validacao local | antes de sobrescrever mudancas locais, alterar bindings ou fazer push/deploy |
...
4. revisar arquivos alterados
5. aplicar somente `git merge --ff-only upstream/main`
6. preservar configuracao privada e bindings reais
7. validar localmente
8. publicar apenas pelo metodo confirmado
