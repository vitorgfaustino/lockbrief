...
- Em repositórios operacionais de Deploy Button/Workers Builds, `wrangler.toml` pode conter IDs reais e deve ser preservado durante atualização.
- Históricos divergentes ou `unrelated histories` não devem ser resolvidos com merge forçado, reset, rebase automático ou push forçado; use o overlay protegido documentado em `docs/ATUALIZACAO.md` ou handoff manual.
